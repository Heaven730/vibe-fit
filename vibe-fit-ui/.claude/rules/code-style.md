...
- 严禁使用 `StyleSheet.create()` 或内联 `style` 对象。所有样式必须通过 `className` 属性现。
- 颜色、间距、字体大小必须映射至 `tailwind.config.js`。
...
- 页面层：统一使用 `app/` 目录，遵循 Expo Router 文件路由规范。
- 私有组件：页面特有的组件存放在同级 `_components/` 目录下。
- 模块化组件：具有业务属性但可复用的组件存放在 `src/components/module[模块名]/`。

## 组件层级引用规则

- `ui/` 禁止引用 store、api、业务类型
- `common/` 可引用 hooks/store，禁止直接调用 api
- `app/[模块]/components/` 禁止被其他模块的文件引用
